import type { ActivityHistoryCardConfig, DirectionOption, TimeRange } from "./types";

const RTL_LANG_PREFIXES = ["ar", "fa", "he", "iw", "ur"];

export function getDomain(entityId: string): string {
  return entityId.includes(".") ? entityId.split(".")[0] ?? entityId : entityId;
}

export function normalizeEntityConfig(entity: string | { entity: string; [key: string]: unknown }) {
  return typeof entity === "string" ? { entity } : entity;
}

export function resolveTimeRange(config: ActivityHistoryCardConfig, now = new Date()): TimeRange {
...
export function isRtl(direction: DirectionOption | undefined, language?: string): boolean {
  if (direction === true || direction === "rtl") return true;
  if (direction === false || direction === "ltr") return false;
  const lang = (language || document.documentElement.lang || navigator.language || "").toLowerCase();
  return RTL_LANG_PREFIXES.some((prefix) => lang === prefix || lang.startsWith(`${prefix}-`));
}
...
export function formatDuration(ms: number): string {
  ...
  if (hours && minutes) return `${hours}:${String(minutes).padStart(2, "0")} שעות`;
...
export function timeToPercent(date: Date, range: TimeRange): number {
  ...
  if (total <= 0) return 0;
  return clamp(((date.getTime() - range.start.getTime()) / total) * 100, 0, 100);
}
