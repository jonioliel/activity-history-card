...
export function summarizeActivity(groups: TimelineGroup[]): ActivitySummary {
  ...
  const now = Date.now();
  const activeNowCount = rows.filter((row) => row.segments.some((segment) => segment.active && segment.start.getTime() <= now && segment.end.getTime() >= now - 90000)).length;
  const lastEvent = [...activeSegments].sort((a, b) => b.start.getTime() - a.start.getTime())[0];
  const lastEventRow = lastEvent ? rows.find((row) => row.entity.entity_id === lastEvent.entity_id) : undefined;
  const mostActiveEntity = [...activeRows].sort((a, b) => b.totalActiveMs - a.totalActiveMs)[0];
  const mostActiveArea = [...groups].filter((group) => group.totalActiveMs > 0).sort((a, b) => b.totalActiveMs - a.totalActiveMs)[0];
