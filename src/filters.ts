...
export function filterRows(
  ...
  return rows.filter((row) => {
    ...
    if (
      filter.areas.length &&
      (!entity.area || !filter.areas.includes(entity.area))
    )
      return false;
    if (filter.domains.length && !filter.domains.includes(entity.domain))
      return false;
    if (search) {
      const haystack = [
        entity.entity_id,
    ...
    if (filter.stateMode === "active_only" && row.totalActiveMs <= 0)
      return false;
...
export function groupRows(
  ...
  for (const row of rows) {
    const key =
      groupBy === "area"
        ? row.entity.area || "ללא אזור"
        : row.entity.domain || "other";
  ...
  return [...groups.entries()]
    .map(([key, groupRowsValue]) =>
      toGroup(
        key,
        groupBy === "domain" ? (DOMAIN_LABELS_HE[key] ?? key) : key,
        groupRowsValue,
      ),
    )
    .sort(
  ...
  const sortedRows = [...rows].sort(
    (a, b) =>
      ...
      Number(
        Boolean(
          b.currentCategory &&
          b.currentCategory !== "off" &&
          b.currentCategory !== "unknown",
        ),
      ) -
        Number(
          Boolean(
            a.currentCategory &&
            a.currentCategory !== "off" &&
            a.currentCategory !== "unknown",
          ),
        ) ||
      a.entity.name.localeCompare(b.entity.name, "he"),
  );
  const totalActiveMs = sortedRows.reduce(
