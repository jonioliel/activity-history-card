export type HistoryRefreshReason = "initial" | "config" | "range" | "entities" | "interval" | "timer" | "manual";
...
export function shouldRefreshFromHassSetter(input: HassRefreshDecisionInput): boolean {
  if (!input.hasFetchedOnce) return true;
  if (!input.live) return false;
  const intervalMs = normalizeRefreshIntervalSeconds(input.refreshIntervalSeconds) * 1000;
