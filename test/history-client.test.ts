...
import { getHistoryRequestPlan, normalizeHistoryResponse } from "../src/history-client";
...
describe("normalizeHistoryResponse", () => {
  it("returns empty history for malformed responses", () => {
    ...
    expect(normalizeHistoryResponse({ "switch.test": "bad" }, ["switch.test"])).toEqual({});
  });

  it("normalizes empty history arrays without throwing", () => {
    expect(normalizeHistoryResponse([[]], ["switch.test"])).toEqual({ "switch.test": [] });
    expect(normalizeHistoryResponse({ "switch.test": [] }, ["switch.test"])).toEqual({ "switch.test": [] });
  });
...
describe("getHistoryRequestPlan", () => {
  it("splits entities that need attributes from minimal history entities", () => {
    ...
    expect(plan.withoutAttributes.map((entity) => entity.entity_id)).toEqual(["switch.test"]);
    expect(plan.withAttributes.map((entity) => entity.entity_id)).toEqual(["climate.living_room"]);
  });
});
